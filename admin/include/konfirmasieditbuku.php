<?php 
    if(isset($_SESSION['id_buku'])){
        ...
        if(empty($id_kategori_buku)) {
            ...
            header("Location:index.php?include=edit-buku&data=$judul&notif =editkosong&jenis=judul");
        }else if(empty($pengarang)) {
            header("Location:index.php?include=edit-buku&data=$pengarang&notif =editkosong&jenis=pengarang");
        }else if(empty($id_penerbit)) {
            header("Location:index.php?include=edit-buku&data=$id_penerbit&notif =editkosong&jenis=penerbit");
        }else if(empty($tahun_terbit)) {
            header("Location:index.php?include=edit-buku&data=$tahun_terbit&notif =editkosong&jenis=tahunterbit");
        }else if(empty($tag)) {
            header("Location:index.php?include=edit-buku&data=$tag&notif=editkosong&jenis=tag");
        }else {
            $lokasi_file = $_FILES['cover']['tmp_name'];
            $nama_file = $_FILES['cover']['name'];
            $direktori = 'cover/'.$nama_file;
