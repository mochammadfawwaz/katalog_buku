    ...
    <section class="content">
            <div class="card">
                ...
                <?php if(!empty($_GET['notif'])) {?>
                  ...
                  <?php if(!empty($_GET['notif'])) { ?>
                    <?php if($_GET['notif'] == "hapusberhasil") { ?>
                      <div class="alert alert-success" role="alert">Data Berhasil Dihapus
                      </div> 
                    <?php } ?>
                  <?php } ?>
                   <!-- Delete User -->
                   <?php
                    if((isset($_GET['aksi'])) && (isset($_GET['data']))) {
                      if($_GET['aksi'] == 'hapus') {
                        $id_user = $_GET['data'];
                        // hapus user
                        $sql_du = "DELETE FROM `user` WHERE `id_user` = '$id_user'";
                        mysqli_query($koneksi, $sql_du);
                      }
                    }
                  ?>
                </div>
                <table class="table table-bordered">
                  ...
                  ?>
                      <tr>
                        ...
                        <td align="center">
                          <a href="index.php?include=edit-user&data=<?php echo $id_user;?>" class="btn btn-xs btn-info"><i class="fas fa-edit"></i> </a>
                          <a href="index.php?include=lihat-user&data=<?php echo $id_user;?>" class="btn btn-xs btn-info"><i class="fas fa-eye"></i> </a>
                          <a href="javascript:if(confirm('Anda yakin ingin menghapus data <?php echo $username; ?>'))window.location.href='index.php?include=user&aksi=hapus&data=<?php echo $id_user; ?>&if=hapusberhasil'" class="btn btn-xs btn-warning"><i class="fas fa-trash" title="Hapus"></i></a>  
                        </td>
                      </tr>
                      <?php   
                      $no++; }
              ...
              <!-- /.card-body -->
              <?php 
                $sql_jum = "SELECT `id_user`,`nama`, `email`, `username`, `password`, `level` FROM `user`";
                if (!empty($katakunci_user)){
              ...
              <div class="card-footer clearfix">
                ...
                <?php 
                    if($jum_halaman == 0) {
                      ...
                      if($halaman != 1) {
                        echo "
                        <li class='page-item'><a class='page-link' href='index.php?include=user&halaman=1'>First</a></li>";
